...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Sleep motion with servo 2 moving first and servo 1 joining after an adjustable trigger ratio"
    )
    ...
    parser.add_argument(
        ...
        help="Maximum seconds to wait before forcing servo 1 to start",
    )
    parser.add_argument(
        "--hold-0",
        type=int,
        help="Optional fixed position for servo 0. Default is current position.",
    )
    parser.add_argument(
        "--hold-3",
        type=int,
        help="Optional fixed position for servo 3. Default is current position.",
    )
    parser.add_argument(
        "--preview",
        action="store_true",
        help="Print the two-stage plan without sending packets",
    )
