...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Lean-forward motion: move servo 1 and 2 to 2148 and 1848"
    )
...
def estimate_wait_seconds(starts: tuple[int, int], targets: tuple[int, int], run_time: int, speed: int, margin: float, max_wait: float) -> float:
    if run_time > 0:
        return min(run_time / 1000.0 + margin, max_wait)
    if speed > 0:
        longest = max(abs(target - start) for start, target in zip(starts, targets))
        return min(longest / speed + margin, max_wait)
    return max_wait
