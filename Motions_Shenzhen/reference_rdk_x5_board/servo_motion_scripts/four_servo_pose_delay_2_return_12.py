...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Run the delayed pose motion, hold for a moment, then slowly return servo 1 and 2 to 2048"
    )
    ...
    parser.add_argument(
        ...
        metavar=("P0", "P1", "P2", "P3"),
        help="Target positions for servo 0/1/2/3 in the first motion",
    )
    parser.add_argument(
        "--speeds",
        nargs=4,
        ...
        metavar=("S0", "S1", "S2", "S3"),
        help="Per-servo speeds for the first motion",
    )
    parser.add_argument(
        "--delay-ratio",
        type=float,
    ...
    parser.add_argument(
        ...
        help="Polling interval while waiting for progress and settle",
    )
    parser.add_argument(
        "--max-wait",
        type=float,
    ...
    parser.add_argument(
        ...
        metavar=("S1", "S2"),
        help="Return speeds for servo 1 and servo 2",
    )
    parser.add_argument(
        "--settle-threshold",
        type=int,
        default=DEFAULT_SETTLE_THRESHOLD,
        help="Position error treated as close enough while waiting for servo 1/2 to settle",
    )
    parser.add_argument(
        "--settle-polls",
        type=int,
        default=DEFAULT_SETTLE_POLLS,
        help="How many consecutive close-enough polls count as settled",
    )
    parser.add_argument(
        "--settle-wait",
        type=float,
        default=DEFAULT_SETTLE_WAIT,
        help="Maximum seconds to wait for servo 1/2 to settle after stage 2 starts",
    )
    parser.add_argument(
        "--stable-delta",
        type=int,
        default=DEFAULT_STABLE_DELTA,
        help="Treat a servo as stable when consecutive reads change by no more than this amount",
    )
...
def wait_until_settled(
    ...
    while True:
        ...
        # Best case: both servos are close enough to the commanded final pose.
        if abs(pos1 - targets[1]) <= settle_threshold and abs(pos2 - targets[2]) <= settle_threshold:
            ...
            if stable_target_count >= settle_polls:
                print("Stage 2 settled near both targets.")
                return last_positions
        else:
            stable_target_count = 0

        # Practical case: servo 2 reached target and the mechanism is no longer moving much.
        if previous_pos1 is not None and previous_pos2 is not None:
            if (
                ...
                if stable_motion_count >= settle_polls:
                    print("Stage 2 settled with servo 2 at target and motion stabilized.")
                    return last_positions
            else:
                stable_motion_count = 0
...
def main() -> int:
    ...
    with serial.Serial(args.device, args.baudrate, timeout=args.timeout) as ser:
        ...
        print(f"Return speeds   : {return_speed_packet}")
        print(f"Return packet   : {return_packet.hex(' ')}")
        print(f"Return sent     : {return_sent}")
        print("Servo 1 is returning to 2150 and servo 2 is returning to 2048.")
