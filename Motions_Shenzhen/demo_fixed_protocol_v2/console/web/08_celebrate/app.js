...
const CELEBRATE_SCENE_ID = "04_offer_celebrate";
const CELEBRATE_API_URL = `/api/run/${CELEBRATE_SCENE_ID}`;
...
async function waitForQueuedResult(queueId) {
  if (!queueId) {
    return null;
  }
  ...
  while (Date.now() - startedAt < 12000) {
    await wait(500);
    const data = await fetchJson(STATE_API_URL);
    const state = data.state || {};
    if (state.lastResult?.queueId === queueId) {
      return state.lastResult;
    }
    const historyMatch = (state.history || []).find((item) => item.queueId === queueId);
    if (historyMatch) {
      return historyMatch;
    }
  }
...
async function triggerCelebrateScene() {
  ...
  // into the Shenzhen fixed-protocol Offer celebration dance scene.
  const queuedResponse = await fetchJson(CELEBRATE_API_URL, {
    method: "POST",
    body: JSON.stringify({
  ...
  const queueId = queuedResponse.queued?.queueId;
  const result = await waitForQueuedResult(queueId);
  if (!result) {
    return;
  }
  if (result.returnCode !== 0) {
    console.warn("Lamp celebrate returned a non-zero result:", result);
  }
}
...
async function playScene() {
  ...
  try {
    ...
    const triggerPromise = triggerCelebrateScene().catch((error) => {
